...
# Description: Implementation of Multiple Models for Zinc Prediction
...
target = "Zinc"
features: list[str] = [
    col for col in data.columns if col not in ["SOC", "Boron", target]
]
...
for model_name, (model, param_grid) in models.items():
    ...
    with mlflow.start_run(run_name=model_name):
        ...
        best_zinc_pipeline_v1: Pipeline = grid_search.best_estimator_
        ...
        mlflow.sklearn.log_model(
            best_zinc_pipeline_v1,
            artifact_path="best_zinc_pipeline_v1",
            input_example=input_example,
        )

        # Make predictions using the trained model
        predictions = best_zinc_pipeline_v1.predict(X_test)
        ...
        if r2 > best_r2_score:
            ...
            best_model_path = Path(
                f"./notebooks/task_3_model_development_and_training/by-Maz/checkpoints/zinc_{model_name}_r2_{best_r2_score}_v1.pkl"
            )
            joblib.dump(best_zinc_pipeline_v1, best_model_path)
